          ...
          <div className="max-w-7xl mx-auto px-6 h-20 md:h-24 flex items-center justify-between">
            ...
            <nav className="hidden lg:flex items-center gap-6 xl:gap-10 h-full">
              {[
                ...
                <motion.div key={item.label} variants={floatingVariant} animate="animate" transition={{ delay: idx * 0.2 }} className="h-full flex items-center">
                  <Link to={item.to} className="relative text-[11px] font-black uppercase tracking-[0.25em] text-slate-300 hover:text-violet-400 transition-all group py-2">
                    {item.label}
              ...
              <motion.div variants={floatingVariant} animate="animate" className="relative group h-full flex items-center">
                <span className="text-[11px] font-black uppercase tracking-[0.25em] text-slate-300 group-hover:text-violet-400 cursor-pointer">About_Us</span>
              ...
              <motion.div variants={floatingVariant} animate="animate" className="relative group h-full flex items-center">
                <span className="text-[11px] font-black uppercase tracking-[0.25em] text-slate-300 group-hover:text-violet-400 cursor-pointer">Services</span>
              ...
              <motion.div variants={floatingVariant} animate="animate" className="shrink-0 ml-4">
                <Link to="/contact" className="inline-block px-6 py-3 bg-transparent text-violet-400 text-[11px] font-black uppercase tracking-[0.25em] relative overflow-hidden group border border-violet-500/40 hover:border-violet-400 transition-all">
                  <span className="relative z-10 transition-colors duration-300 group-hover:text-white">Initialize_Contact</span>
            ...
            <button 
              ...
              className="lg:hidden text-violet-500 text-3xl p-2"
            >
              ☰
            </button>
          </div>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="fixed inset-0 z-[100] bg-[#030712] flex flex-col lg:hidden overflow-y-auto"
          >
            <div className="p-6 flex items-center justify-between border-b border-violet-500/20">
              <img src={logo} alt="Logo" className="h-8 w-auto" />
              <button onClick={() => setMobileOpen(false)} className="text-violet-500 text-4xl">✕</button>
            </div>
            ...
            <nav className="flex flex-col p-8 gap-6">
              {[
                ...
                <Link 
                  ...
                  className="text-lg font-black uppercase tracking-widest text-slate-300 border-b border-white/5 pb-2"
                >
                  {link.label}
                </Link>
              ))}
              ...
              <div className="mt-4">
                <p className="text-[10px] font-mono text-violet-500 mb-4 tracking-[0.4em] uppercase">Services</p>
                <div className="grid grid-cols-1 gap-3">
                  {services.map((s) => (
                    <Link 
                      ...
                      to={s.to} 
                      onClick={() => setMobileOpen(false)}
                      className="text-xs font-bold text-slate-400 uppercase tracking-wider hover:text-violet-400"
                    >
                      {s.name}
                    </Link>
                  ))}
